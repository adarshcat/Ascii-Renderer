...
void transformRotateCube(Triangle *triangles, Triangle *base, unsigned int totalTriangles, float time){
    ...
    for (int i=0; i<totalTriangles; i++){
        ...
        vec4(tempVec4, base[i].v1.position[0], base[i].v1.position[1], base[i].v1.position[2], 1.0);
        vec4_multiply_mat4(t1, tempVec4, rotationMatrix);

        vec4(tempVec4, base[i].v2.position[0], base[i].v2.position[1], base[i].v2.position[2], 1.0);
        vec4_multiply_mat4(t2, tempVec4, rotationMatrix);

        vec4(tempVec4, base[i].v3.position[0], base[i].v3.position[1], base[i].v3.position[2], 1.0);
        vec4_multiply_mat4(t3, tempVec4, rotationMatrix);
        ...
        vec4(tempVec4, base[i].v1.normal[0], base[i].v1.normal[1], base[i].v1.normal[2], 0.0);
        vec4_multiply_mat4(t1, tempVec4, rotationMatrix);

        vec4(tempVec4, base[i].v2.normal[0], base[i].v2.normal[1], base[i].v2.normal[2], 0.0);
        vec4_multiply_mat4(t2, tempVec4, rotationMatrix);

        vec4(tempVec4, base[i].v3.normal[0], base[i].v3.normal[1], base[i].v3.normal[2], 0.0);
        vec4_multiply_mat4(t3, tempVec4, rotationMatrix);
